...
import { findParentAndIndex, getLogicalChildren, findPathToNode, getAllNodesInPreOrder, getLeafNodesInPreOrder, getPreviousLeaf, isEmptyNode, findNodeById } from '../utils/treeUtils';
...
const MathEditor: React.FC = () => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    //         const newContent = node.content + append;
    //         console.log(`Updating node content: was "${node.content}", appending "${append}"`);

    //         // If the new content contains a backslash, check if it's forming a special sequence
    //         if (newContent.includes('\\')) {
    //           const match = specialSequences.find(seq => seq.sequence === newContent);
    //           if (match) {
    //             const replacement = match.mathNode;

    //             // If the replacement node has children, select the first one (e.g., for accented nodes)
    //             if ('children' in replacement && replacement.children.length > 0) {
    //               setSelectedId(replacement.children[0].id);
    //             } else {
    //               // For standalone symbols like Greek letters, clear selection
    //               console.log(`${replacement} exists but idk where to select now. Selecting null...`)
    //               setSelectedId(null);
    //             }
    ...
    //           // No complete match found yet — continue building the sequence
    //           return { ...node, content: newContent };
    //         }

    //         // Handle case: if the new character is a digit
    //         const isDigit = /^[0-9]$/.test(append);
    //         const isAllDigits = /^[0-9]*$/.test(node.content);

    //         if (isDigit && isAllDigits) {
    //           // If the entire content is digits, extend current node
    //           return { ...node, content: newContent };
    //         }

    //         // Otherwise, insert a new text node after this one (to split semantic groups)
    //         const pos = findParentAndIndex(prev, node.id);
    //         if (!pos || !pos.parent) {
    //           console.log(`no pos bro`)
    //           return node;
    //         }
    ...
    //         const newTextNode = createTextNode(append); // New node starts with the appended character

    //         const updatedChildren = [
    //           ...children.slice(0, pos.index + 1),
    //           newTextNode,
    //           ...children.slice(pos.index + 1),
    //         ];

    //         // Reconstruct the parent node with updated children list
    //         const updatedParent = {
    //           ...parent,
    //           children: updatedChildren,
    //         };

    //         // Replace the parent in the full tree and select the new node
    //         setSelectedId(newTextNode.id);
    //         const updatedRoot = findAndUpdate(prev, parent.id, () => updatedParent);
    //         return newTextNode;
    //       }
    //       //TODO maybe this needs to be changed. is this where I ensure that typing e.g. at start doesnt fuck up?
    //       else if (node.type === 'inline-container') {
    //         console.log(`${nodeToLatex(node)} is somehow the node right now`)
    ...
    const updateMathNode = (append: string) => {
      ...
      let newSelectedNode = findNodeById(rootNode, newSelectedId) | rootNode
      console.log(nodeToLatex(newSelectedNode))
      console.log(`${newSelectedNode?.type}`)

      // if (newSelectedNode?.type != 'text') {
      //   console.log(`yes this is reached`)
      //   const preOrder = getAllNodesInPreOrder(newSelectedNode)
      //   console.log(`preorder ${preOrder.map(x => nodeToString(x))}`)
      //   newSelectedNode = preOrder.find(x => x.type === 'text')
      //   //newSelectedId = selectedNode.children[selectedNode.children.length - 1].id
      //   setSelectedFirstLeaf(newSelectedNode)
      // }
      ...
      setRootNode(prev => {
        ...
        let parentToReplace: MathNode | null = null;
        let updatedChildren: MathNode[] | null = null;
    
        const recursiveUpdate = (node: MathNode): MathNode => {
          if (found) return node;
    
          if (node.id === newSelectedId && node.type === 'text') {
            const newContent = node.content + append;
            const isDigit = /^[0-9]$/.test(append);
            const isAlpha = /^[a-zA-Z]$/.test(append);
    
            // === Case 1: Special sequence match ===
            if (node.content.includes("\\")) {
              console.log(`case: special sequence incomplete`)

              const match = specialSequences.find(seq => seq.sequence === newContent);
              if (match) {
                console.log(`case: special sequence match found`)

                const replacement = match.mathNode;
                newSelectedNode =
                  "children" in replacement && replacement.children.length > 0
                    ? replacement.children[0]
                    : prev;
                newSelectedId = newSelectedNode.id;
                found = true;
                return replacement;
              } else {
                console.log(`case: no special sequence match found yet`)

                // Partial sequence: keep appending
                found = true;
                return { ...node, content: newContent };
              }
            }
            ...
            const isAllDigits = /^[0-9]*$/.test(node.content);
            if (isDigit && isAllDigits) {
              console.log(`case: add digit to number`)
              found = true;
              return { ...node, content: newContent };
            }
    
            // === Case 3 & 4: Letter or Other — split ===
            const parentInfo = findParentAndIndex(prev, node.id);
            if (!parentInfo || !parentInfo.parent) {
              console.warn("Cannot split: parent not found");
              found = true;
              return node;
            }
            console.log(`case: none of the ifs worked`)

            const { parent, index } = parentInfo;
            const siblings = getLogicalChildren(parent);
            const newTextNode = createTextNode(append);
            newSelectedNode = newTextNode
            newSelectedId = newTextNode.id;
    
            // Mark parent for replacement
            updatedChildren = [
              ...siblings.slice(0, index + 1),
              newTextNode,
              ...siblings.slice(index + 1),
            ];
            parentToReplace = parent;
    
            found = true;
            return node; // Return the same node — the change will be made at the parent level
          }
    
          if ('children' in node && Array.isArray(node.children)) {
            console.log(`case: ${node.type} has children? Oops. Going to select its last child`)
            newSelectedId = node.children[node.children.length - 1].id
            newSelectedNode = node.children[node.children.length - 1]
            
            console.log(`${newSelectedId} is the let, but the const is ${selectedId}`)
                     
            const newChildren = node.children.map(child => recursiveUpdate(child));
            return { ...node, children: newChildren };
          }
        ...
        const applyParentReplacement = (node: MathNode): MathNode => {
          if (!parentToReplace || !updatedChildren) return node;
          if (node.id === parentToReplace.id) {
            console.log(`node id == parenttoreplace id`)

            return { ...node, children: updatedChildren };
          }
          if ('children' in node && Array.isArray(node.children)) {
            console.log(`has children in apply replace`)

            const newChildren = node.children.map(child => applyParentReplacement(child));
            return { ...node, children: newChildren };
          }
          return node;
        };
    
        const updatedTree = applyParentReplacement(recursiveUpdate(prev));
        setSelectedFirstLeaf(newSelectedNode);
        return updatedTree;
      });
    };
    ...
    switch (e.key) {
      ...
      case '/':
        ...
        setRootNode(prev => {
          ...
          setSelectedFirstLeaf(transformed.denominator);
        ...
        setRootNode(prev => {
          ...
          if (currentNode.type === "inline-container") {
            ...
            if (currentNode.children.every(isEmptyNode)) {
              ...
              // Delete this child
              return findAndUpdate(prev, currentNode.id, () => createTextNode()); 
            }
            else {
  ...
  const setSelectedFirstLeaf = (node: MathNode) => {
    const leaves = getLeafNodesInPreOrder(node)
