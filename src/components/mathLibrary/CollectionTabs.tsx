...
//     setCollections(colls =>
//       colls.map(col => (col.id === id ? { ...col, archived: true } : col))
//     );
//     if (id === activeColl) {
...
//   // Dropping math nodes into inactive tabs
//   const onTabDropEntry = (e: React.DragEvent, collectionId: string) => {
//     // NOTE: IS THIS WORKING? IS IT DOING ANYTHING? I DON'T THINK SO? AT LEAST THE RESULT IS NOT VISIBLE
//     e.preventDefault();
//     const data = e.dataTransfer.getData("text/plain");
//     console.warn(`Drop Entry in Tab? ${data}`)
//     if (!data) return;
...
//   const onTabDragOverEntry = (e: React.DragEvent) => {
//     // Allow drop of math entry
//     if (e.dataTransfer.types.includes("application/x-math-entry")) {
//       e.preventDefault();
//     }
//   };
...
//               onDragOver={e => {
//                 onTabDragOver(e, idx);
//                 onTabDragOverEntry(e); // new, for accepting drops of math entries
//               }}
//               onDrop={e => {
//                 onTabDrop(e, idx);
//                 onTabDropEntry(e, c.id); // new: if it's a math entry drop, handle that
//               }}
...
//           <button className={styles.archiveButton} onClick={() => setMenuOpenFor("archive")}>🗂️</button>
//         </Tooltip>
//       </div>
//     </div>
//   );
...
const CollectionTabs: React.FC<CollectionTabsProps> = ({
  ...
  const archiveCollection = (id: string) => {
    setCollections((colls) => colls.map((col) => (col.id === id ? { ...col, archived: true } : col)));
    if (id === activeColl) {
      const next = collections.find((c) => c.id !== id && !c.archived);
      if (next) setActiveColl(next.id);
      else setActiveColl("");
